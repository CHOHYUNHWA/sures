...
import { DateSelector, TimeGrid, ConsultationTypeGrid, adminReservationApi } from '@/features/reservation'
import type { ConsultationType, CreateReservationRequest } from '@/shared/types'
import styles from './ReservationCreate.module.css'

export function AdminReservationCreatePage() {
  ...
  const handleDateSelect = useCallback(async (date: string) => {
    ...
    try {
      const times = await adminReservationApi.getReservedTimes(date)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const reservation = await adminReservationApi.create(request)
      navigate(`/admin/reservations/${reservation.id}`)
    } catch (err) {
      setError('예약 등록에 실패했습니다. 다시 시도해주세요.')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className={styles.container}>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-actions">
          <Link to="/admin/reservations" style={{ flex: 1 }}>
